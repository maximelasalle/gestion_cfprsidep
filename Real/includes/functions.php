...
function login($email, $password, $mysqli) {
    ...
    if ($stmt = $mysqli->prepare(" SELECT members.id, members.name, surname, 
										password, salt, role_id, groupe_id, groupe.name 
		  FROM members, groupe 
		 WHERE email = ?
		   AND members.groupe_id = groupe.id
	   LIMIT 1")) {
        ...
        $stmt->bind_result($user_id,$name,$surname, $db_password, $salt, $role_id, $group_id, $group_name);
        $stmt->fetch();
        ...
        if ($stmt->num_rows == 1) {
            ...
            if (checkbrute($user_id, $mysqli) == true) {
                ...
                if ($db_password == $password) {
					...
					$mysqli->query("INSERT INTO broker_a(member_id,transaction_code,field_1)
